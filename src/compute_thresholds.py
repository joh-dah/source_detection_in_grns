...
#     processed_dir = const.PROCESSED_PATH
...
def compute_and_store_thresholds(datasets=None):
    ...
    if datasets is None:
        # Mode: Called without datasets (from command line)
        data_backward_path = processed_path / "data_backward.pt"
        data_forward_path = processed_path / "data_forward.pt"
        ...
        if not datasets:
            raise FileNotFoundError(f"No processed data files found in {processed_path}")

    assert processed_path is not None, "processed_path must be determined"
    assert datasets is not None, "datasets must be determined"
    
    # Check if thresholds already exist
    thresholds_path = processed_path / "thresholds.pt"
    ...
    # Load backward data efficiently
    if "backward" in datasets:
        backward_file = processed_path / "data_backward.pt"
        if backward_file.exists():
            print("Loading backward data for threshold computation...")
            backward_data_list = torch.load(backward_file, weights_only=False)
            print(f"Processing {len(backward_data_list)} backward samples for thresholds...")
    ...
    # Load forward data efficiently
    if "forward" in datasets:
        forward_file = processed_path / "data_forward.pt"
        if forward_file.exists():
            print("Loading forward data for threshold computation...")
            forward_data_list = torch.load(forward_file, weights_only=False)
            print(f"Processing {len(forward_data_list)} forward samples for thresholds...")
    ...
    def compute_thresholds(values):
        ...
        print(f"Computing thresholds from {len(values)} expression values")
    ...
    # For backward direction, combine diseased and treated
    if all_diseased and all_treated:
        backward_values = all_diseased + all_treated
        thresholds["backward"] = compute_thresholds(backward_values)
        print(f"Backward thresholds: min={thresholds['backward'].min():.3f}, max={thresholds['backward'].max():.3f}")
...
def main():
    ...
    #compute_and_store_thresholds(experiment_name, args.force)
